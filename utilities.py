...
def argmax_new(f, gen):
   """takes a unary function f and generator gen.
      returns a list of the values generated by gen for which f is maximal.
   """
   return reduce(lambda acc, x: x if not acc or x[1] > acc[1] else acc, \
            map(lambda x: (x, f(x)), gen), None)
